...
ForcePoint SHALL không còn dùng `targetRigidbody` cố định. Thay vào đó, mỗi khi `ApplyForce()` được gọi, SHALL gọi `FindLeftmostLeaf()` để lấy Rigidbody2D của leaf limb ở bên trái nhất, rồi apply `new Vector2(forceX, forceY)` với `ForceMode2D.Impulse` lên target đó.
